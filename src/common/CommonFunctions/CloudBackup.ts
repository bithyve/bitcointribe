...
export default class CloudBackup {
  ...
  constructor( stateVars?: {
    ...
    const { recoveryCallback, share, callBack, dataObject, googlePermissionCall, googleCloudLoginCallback } = stateVars
  ...
  public GoogleDriveLogin = ( params: {
    ...
    try {
      const { checkDataIsBackedup, share, googlePermissionCall, googleCloudLoginCallback } = params
        ...
        .then( () => {
          GoogleDrive.login( async ( err, data ) => {
            ...
            if ( !googlePermissionCall )
              this.handleLogin( {
                err, data, checkDataIsBackedup, share
              } )
            else{
  ...
  public handleLogin = async ( params: {
    ...
    try {
      ...
      const { err, data, checkDataIsBackedup, share } = params
      const result = err || data
