        ...
        <View style={{ height: '100%' }}>
            <View style={{ flexDirection: 'row', padding: wp('7%') }}>
                <View style={{ flex: 3, justifyContent: 'center' }}>
                    ...
                    <Text style={{ ...styles.modalInfoText, marginTop: wp('1.5%') }}>To recover your wallet you have to select the security question and enter its answer</Text>
                </View>
                <View style={{ flex: 1, }}>
                    <KnowMoreButton onpress={() => props.onPressKnowMore()} containerStyle={{ marginLeft: 'auto', marginTop: 10 }} textStyle={{}} />
                </View>
            </View>
            <View style={{ paddingLeft: wp('6%'), paddingRight: wp('6%'), }}>
                ...
                <View style={{ position: 'relative', }}>
                        ...
                        <View style={styles.dropdownBoxModal}>
                            <ScrollView>
                                {dropdownBoxList.map((value, index) =>
                                    <TouchableOpacity onPress={() => { setDropdownBoxValue(value); setDropdownBoxOpenClose(false); props.onQuestionSelect(dropdownBoxValue); }} style={{
                    ...
                    <TextInput
                        style={{ ...styles.inputBox, width: '100%', marginTop: 15, marginBottom: hp('6%'), }}
                        placeholder={'Enter Security Answer'}
                        placeholderTextColor={Colors.borderColor}
                        value={answer}
                        onChangeText={(text) => {setAnswer(text); props.onTextChange(answer);}}
                        onFocus={() => { props.bottomSheetRef.current.snapTo(2); setDropdownBoxOpenClose(false); }}
                        onBlur={() => { props.bottomSheetRef.current.snapTo(1); setDropdownBoxOpenClose(false) }}
                    />
                    <Text style={styles.modalInfoText}>The Security Answer is case sensitive, make sure you{"\n"}enter the case, numeric or symbolic values correctly</Text>
                </View>
                <TouchableOpacity
