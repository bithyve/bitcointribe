...
class Wallet extends React.Component {
  ...
  connnection_FetchData = async () => {
    try {
      ...
      const resAccount = await comFunDBRead.readTblAccount();
      const temp = [];
      for (let i = 0; i < resAccount.length; i++) {
        const dataAccount = resAccount[i];
        const additionalInfo = JSON.parse(dataAccount.additionalInfo);
        // console.log( { additionalInfo } );
        const setupData = additionalInfo != '' ? additionalInfo[0] : '';
  ...
  getDeepLinkingData() {
    try {
      ...
      if (urlType != '') {
        if (
          urlType == 'SSS Recovery SMS/EMAIL' ||
          urlType == 'SSS Recovery QR'
        ) {
          console.log('email');
          this.setState({
  ...
  click_SkipAssociateContact = async () => {
    try {
      const deepLinkingUrlType = utils.getDeepLinkingType();
  ...
  generateSSSDetails = async () => {
    try {
      ...
      const setUpWalletAnswerDetails = JSON.parse(
      ...
      const sss = await bitcoinClassState.getS3ServiceClassState();
      let resSetupSecureAccount = await secureAccount.setupSecureAccount();
      if (resSetupSecureAccount.status == 200) {
        resSetupSecureAccount = resSetupSecureAccount.data;
      } else {
        alert.simpleOkAction(
          'Oops',
          resSetupSecureAccount.err,
          this.click_StopLoader,
        );
      }
      console.log({ resSetupSecureAccount });
      let secondaryXpub = await secureAccount.getSecondaryXpub();
      ...
      let getSecoundMnemonic = await secureAccount.getRecoveryMnemonic();
      if (getSecoundMnemonic.status == 200) {
        ...
        alert.simpleOkAction(
          'Oops',
          getSecoundMnemonic.err,
          this.click_StopLoader,
        );
      }
      console.log({ getSecoundMnemonic });
      // Get Shares
      const generateShareRes = await sss.generateShares(
        setUpWalletAnswerDetails[0].Answer,
      );
      ...
      if (generateShareRes.status == 200) {
        ...
        const resInitializeHealthcheck = await sss.initializeHealthcheck(
          autoHealthShares,
        );
        console.log({ resInitializeHealthcheck });
        if (
          resInitializeHealthcheck.status == 200 ||
          resInitializeHealthcheck.status == 400
        ) {
          const shareIds = [];
          // console.log( { autoHealthShares } );
          ...
          if (resEncryptSocialStaticNonPMDD.status == 200) {
            resEncryptSocialStaticNonPMDD =
              resEncryptSocialStaticNonPMDD.data.encryptedStaticNonPMDD;
            const buddyStaticNonPMDD = {
              getSecoundMnemonic,
              twoFASecret: resSetupSecureAccount.setupData.secret,
              secondaryXpub,
              bhXpub: resSetupSecureAccount.setupData.bhXpub,
            };
            console.log({ buddyStaticNonPMDD });
            let resEncryptBuddyStaticNonPMDD = await sss.encryptStaticNonPMDD(
              buddyStaticNonPMDD,
            );
            if (resEncryptBuddyStaticNonPMDD.status == 200) {
              resEncryptBuddyStaticNonPMDD =
                resEncryptBuddyStaticNonPMDD.data.encryptedStaticNonPMDD;
              const rescreateMetaShare = await sss.createMetaShare(
                1,
                encryptedShares[0],
                resEncryptSocialStaticNonPMDD,
              ...
              console.log({
                encpShare: encryptedShares[1],
                rescreateMetaShare,
              });
              const resGenerateEncryptedMetaShare1 = await sss.generateEncryptedMetaShare(
                rescreateMetaShare.data.metaShare,
              );
              const rescreateMetaShare1 = await sss.createMetaShare(
                2,
                encryptedShares[1],
                resEncryptSocialStaticNonPMDD,
                walletDetails.walletType,
              ...
              console.log({
                encpShare: encryptedShares[2],
                rescreateMetaShare1,
              });
              const resGenerateEncryptedMetaShare2 = await sss.generateEncryptedMetaShare(
                rescreateMetaShare1.data.metaShare,
              );
              const rescreateMetaShare2 = await sss.createMetaShare(
                3,
                encryptedShares[2],
                resEncryptBuddyStaticNonPMDD,
                walletDetails.walletType,
              );
              const resGenerateEncryptedMetaShare3 = await sss.generateEncryptedMetaShare(
                rescreateMetaShare2.data.metaShare,
              );
              console.log({ rescreateMetaShare2 });
              // for pdf
              const rescreateMetaShare3 = await sss.createMetaShare(
                4,
                encryptedShares[3],
                resEncryptBuddyStaticNonPMDD,
                walletDetails.walletType,
              ...
              if (rescreateMetaShare3.status == 200) {
                ...
                if (qrcode4share.status == 200) {
                  ...
                  const temp = [];
                  temp.push({
                    arrQRCodeData: qrcode4share,
                    secondaryXpub,
                    qrData: resSetupSecureAccount.setupData.qrData,
                    secret: resSetupSecureAccount.setupData.secret,
                    secondaryMnemonic: getSecoundMnemonic,
                    bhXpub: resSetupSecureAccount.setupData.bhXpub,
                  });
                  const resGenerate4thsharepdf = await this.generate4thShare(
                    temp,
                    setUpWalletAnswerDetails[0].Answer,
                    walletDetails,
                  );
                  ...
                  if (resGenerate4thsharepdf != '') {
                    const rescreateMetaShare4 = await sss.createMetaShare(
                      5,
                      encryptedShares[4],
                      resEncryptBuddyStaticNonPMDD,
                      walletDetails.walletType,
                    ...
                    if (rescreateMetaShare4.status == 200) {
                      let qrcode5share = await sss.createQR(
                        rescreateMetaShare4.data.metaShare,
                        5,
                      );
                      ...
                      if (qrcode5share.status == 200) {
                        ...
                        console.log({ temp });
                        const resGenerate5thsharepdf = await this.generate5thShare(
                          temp,
                          setUpWalletAnswerDetails[0].Answer,
                          walletDetails,
                        );
                        console.log({
                          resGenerate5thsharepdf,
                        });
                        if (resGenerate5thsharepdf != '') {
                          ...
                          const encryptedMetaShare = [
                            {
                              metaShare: rescreateMetaShare.data.metaShare,
                            },
                            {
                              metaShare: rescreateMetaShare1.data.metaShare,
                            },
                            {
                              metaShare: rescreateMetaShare2.data.metaShare,
                            },
                            {
                              metaShare: resGenerate4thsharepdf,
                            },
                            {
                              metaShare: resGenerate5thsharepdf,
                            },
                          ];
                          const arrTypes = [
                            {
              ...
              alert.simpleOkAction(
                'Oops',
                resEncryptBuddyStaticNonPMDD.err,
                this.click_StopLoader,
              );
            }
          } else {
            alert.simpleOkAction(
              'Oops',
              resEncryptSocialStaticNonPMDD.err,
              this.click_StopLoader,
            );
          }
        } else {
  ...
  generate4thShare = async (
    ...
    try {
      ...
      return new Promise(async (resolve, reject) => {
        data = data[0];
        const { arrQRCodeData } = data;
        const { secondaryXpub } = data;
        const { qrData } = data;
        // set state value for qrcode
        this.setState(
          {
            qrcodeImageString1: await this.getCorrectFormatStirng(
              arrQRCodeData[0],
            ),
            qrcodeImageString2: await this.getCorrectFormatStirng(
              arrQRCodeData[1],
            ),
            qrcodeImageString3: await this.getCorrectFormatStirng(
              arrQRCodeData[2],
            ),
            qrcodeImageString4: await this.getCorrectFormatStirng(
              arrQRCodeData[3],
            ),
            qrcodeImageString5: await this.getCorrectFormatStirng(
              arrQRCodeData[4],
            ),
            qrcodeImageString6: await this.getCorrectFormatStirng(
              arrQRCodeData[5],
            ),
            qrcodeImageString7: await this.getCorrectFormatStirng(
              arrQRCodeData[6],
            ),
            qrcodeImageString8: await this.getCorrectFormatStirng(
              arrQRCodeData[7],
            ),
          ...
          async () => {
            ...
            setTimeout(async () => {
              const { base64string1 } = this.state;
              const { base64string2 } = this.state;
              const { base64string3 } = this.state;
              const { base64string4 } = this.state;
              const { base64string5 } = this.state;
              const { base64string6 } = this.state;
              const { base64string7 } = this.state;
              const { base64string8 } = this.state;
              const { base64string9 } = this.state;
              const { base64string10 } = this.state;
              const res4thShare1Create = await this.generateSahreQRCode(
                base64string1,
                'qrcode4thSahre1.png',
              );
              // console.log( { res4thShare1Create } );
              const res4thShare2Create = await this.generateSahreQRCode(
                base64string2,
                'qrcode4thSahre2.png',
              );
              //    console.log( { res4thShare2Create } );
              const res4thShare3Create = await this.generateSahreQRCode(
                base64string3,
                'qrcode4thSahre3.png',
              );
              //  console.log( { res4thShare3Create } );
              const res4thShare4Create = await this.generateSahreQRCode(
                base64string4,
                'qrcode4thSahre4.png',
              );
              // console.log( { res4thShare4Create } );
              const res4thShare5Create = await this.generateSahreQRCode(
                base64string5,
                'qrcode4thSahre5.png',
              );
              // console.log( { res4thShare5Create } );
              const res4thShare6Create = await this.generateSahreQRCode(
                base64string6,
                'qrcode4thSahre6.png',
              );
              // console.log( { res4thShare6Create } );
              const res4thShare7Create = await this.generateSahreQRCode(
                base64string7,
                'qrcode4thSahre7.png',
              );
              // console.log( { res4thShare7Create } );
              const res4thShare8Create = await this.generateSahreQRCode(
                base64string8,
                'qrcode4thSahre8.png',
              );
              // console.log( { res4thShare8Create } );
              const resSecoundXpub4Share = await this.generateXpubAnd2FAQRCode(
                base64string9,
                'secoundryXpub4Share.png',
              );
              // console.log( { resSecoundXpub4Share } );
              const res2FASecret4Share = await this.generateXpubAnd2FAQRCode(
                base64string10,
                'googleAuto2FASecret4Share.png',
              );
              // console.log( { res2FASecret4Share } );
              let create4thPdf;
              if (Platform.OS == 'android') {
                create4thPdf = await this.genreatePdf(
                  data,
                  '/storage/emulated/0/qrcode4thSahre1.png',
                  '/storage/emulated/0/qrcode4thSahre2.png',
                  '/storage/emulated/0/qrcode4thSahre3.png',
                  '/storage/emulated/0/qrcode4thSahre4.png',
                  '/storage/emulated/0/qrcode4thSahre5.png',
                  '/storage/emulated/0/qrcode4thSahre6.png',
                  '/storage/emulated/0/qrcode4thSahre7.png',
                  '/storage/emulated/0/qrcode4thSahre8.png',
                  '/storage/emulated/0/secoundryXpub4Share.png',
                  '/storage/emulated/0/googleAuto2FASecret4Share.png',
                  `${
                    walletDetails.walletType.split(' ')[0]
                  } Hexa Wallet Share 4.pdf`,
                  `${
                    walletDetails.walletType.split(' ')[0]
                  } Hexa Wallet Share 4`,
                  password,
        ...
        this.setState(
          ...
          async () => {
            ...
            setTimeout(async () => {
              ...
              const { base64string9 } = this.state;
              const { base64string10 } = this.state;
              const res5thShare1Create = await this.generateSahreQRCode(
                base64string1,
                'qrcode5thSahre1.png',
              );
              console.log({ res5thShare1Create });
              const res5thShare2Create = await this.generateSahreQRCode(
                base64string2,
                'qrcode5thSahre2.png',
              );
              //    console.log( { res4thShare2Create } );
              const res5thShare3Create = await this.generateSahreQRCode(
                base64string3,
                'qrcode5thSahre3.png',
              );
              //  console.log( { res4thShare3Create } );
              const res5thShare4Create = await this.generateSahreQRCode(
                base64string4,
                'qrcode5thSahre4.png',
              );
              // console.log( { res4thShare4Create } );
              const res5thShare5Create = await this.generateSahreQRCode(
                base64string5,
                'qrcode5thSahre5.png',
              );
              // console.log( { res4thShare5Create } );
              const res5thShare6Create = await this.generateSahreQRCode(
                base64string6,
                'qrcode5thSahre6.png',
              );
              // console.log( { res5thShare6Create } );
              const res5thShare7Create = await this.generateSahreQRCode(
                base64string7,
                'qrcode5thSahre7.png',
              );
              // console.log( { res5thShare7Create } );
              const res5thShare8Create = await this.generateSahreQRCode(
                base64string8,
                'qrcode5thSahre8.png',
              );
              // console.log( { res5thShare8Create } );
              const resSecoundXpub5Share = await this.generateXpubAnd2FAQRCode(
                base64string9,
                'secoundryXpub5Share.png',
              );
              // console.log( { resSecoundXpub4Share } );
              const res2FASecret5Share = await this.generateXpubAnd2FAQRCode(
                base64string10,
                'googleAuto2FASecret5Share.png',
              );
              // console.log( { res2FASecret4Share } );
              let create5thPdf;
              if (Platform.OS == 'android') {
                create5thPdf = await this.genreatePdf(
                  data,
                  '/storage/emulated/0/qrcode5thSahre1.png',
                  '/storage/emulated/0/qrcode5thSahre2.png',
                  '/storage/emulated/0/qrcode5thSahre3.png',
                  '/storage/emulated/0/qrcode5thSahre4.png',
                  '/storage/emulated/0/qrcode5thSahre5.png',
                  '/storage/emulated/0/qrcode5thSahre6.png',
                  '/storage/emulated/0/qrcode5thSahre7.png',
                  '/storage/emulated/0/qrcode5thSahre8.png',
                  '/storage/emulated/0/secoundryXpub5Share.png',
                  '/storage/emulated/0/googleAuto2FASecret5Share.png',
                  `${
                    walletDetails.walletType.split(' ')[0]
                  } Hexa Wallet Share 5.pdf`,
                  `${
                    walletDetails.walletType.split(' ')[0]
                  } Hexa Wallet Share 5`,
                  password,
  ...
  genreatePdf = async (
    ...
    pathShare7: string,
    pathShare8: string,
    pathSecoundXpub: string,
    path2FASecret: string,
    pdfFileName: string,
    forShare: string,
    password: string,
  ) => {
    try {
      return new Promise(async (resolve, reject) => {
        ...
        console.log({ password });
        const { arrQRCodeData } = data;
        const secret2FA = data.secret;
        const { secondaryMnemonic } = data;
        const { bhXpub } = data;
        // Share 1
        // let arrShare1 = .split();
        // console.log( { arrShare1 } );
        const arrShare1 = this.chunkArray(arrQRCodeData[0], 7);
        const arrShare2 = this.chunkArray(arrQRCodeData[1], 7);
        const arrShare3 = this.chunkArray(arrQRCodeData[2], 7);
        const arrShare4 = this.chunkArray(arrQRCodeData[3], 7);
        const arrShare5 = this.chunkArray(arrQRCodeData[4], 7);
        const arrShare6 = this.chunkArray(arrQRCodeData[5], 7);
        const arrShare7 = this.chunkArray(arrQRCodeData[6], 7);
        const arrShare8 = this.chunkArray(arrQRCodeData[7], 7);
        // Secound Mnemonic
        const arrSecondaryMnemonic = secondaryMnemonic.split(' ');
        let firstArrSecondaryMnemonic;
        let secoundArrSecondaryMnemonic;
        let threeSecondaryMnemonic;
        const arrSepArray = this.chunkArray(arrSecondaryMnemonic, 3);
        firstArrSecondaryMnemonic = arrSepArray[0].toString();
        firstArrSecondaryMnemonic = firstArrSecondaryMnemonic
          .split(',')
          .join(' ');
        ...
        // console.log( { bhXpub } );
        let firstArrbhXpub;
        let secoundArrbhXpub;
        let threebhXpub;
        const arrSepArraybhXpub = bhXpub.match(/.{1,40}/g);
        ...
        const page3 = PDFPage.create()
          .drawText('Part 5', {
            x: 5,
            y: 470,
            fontSize: 10,
          })
          .drawImage(pathShare5, 'png', {
            x: 40,
            y: 320,
            width: 160,
            height: 140,
        ...
        const page4 = PDFPage.create()
          .drawText('Part 7', {
            x: 5,
            y: 470,
            fontSize: 10,
          })
          .drawImage(pathShare7, 'png', {
            x: 40,
            y: 320,
            width: 160,
            height: 140,
        ...
        const page5 = PDFPage.create()
          .drawText('Secondary Xpub (Encrypted):', {
            x: 5,
            y: 480,
            fontSize: 18,
          })
          .drawImage(pathSecoundXpub, 'png', {
            x: 25,
            y: 300,
            width: 160,
            height: 140,
        ...
        const page6 = PDFPage.create()
          .drawText('2FA Secret:', {
            x: 5,
            y: 480,
            fontSize: 18,
          })
          .drawImage(path2FASecret, 'png', {
            x: 25,
            y: 300,
            width: 160,
            height: 140,
  ...
  getBalReularAccount = async () => {
    try {
      ...
      let getBalR = await regularAccount.getBalance();
      console.log({ getBalR });
  ...
  getBalSecureAccount = async () => {
    try {
      ...
      let getBalS = await secureAccount.getBalance();
      console.log({ getBalS });
  ...
  checkHealthStatus = async () => {
    try {
      ...
      share.selfshareShareDate2 =
        sssDetails[3].acceptedDate != '' ? sssDetails[3].acceptedDate : 0;
      share.selfshareShareShareId2 =
        sssDetails[3].shareId != '' ? sssDetails[3].shareId : null;
      share.selfshareShareDate3 =
        sssDetails[4].acceptedDate != '' ? sssDetails[4].acceptedDate : 0;
      share.selfshareShareId3 =
        sssDetails[4].shareId != '' ? sssDetails[4].shareId : null;
  ...
  setHealthCheakIcon = async () => {
    try {
      ...
      const appHealth = JSON.parse(walletDetails.appHealthStatus);
      ...
      if (appHealth.overallStatus == '1') {
        this.setState({
          ...
          arr_CustShiledIcon: [
            {
              title:
                'The wallet backup is not secured. Please complete the setup to safeguard against loss of funds',
              image: 'sheild_1',
        ...
        this.setState({
          ...
          arr_CustShiledIcon: [
            {
              title:
                'Great!! The wallet backup is secure. Keep an eye on the health of the backup here',
              image: 'sheild_5',
              imageHeight: this.animatedShieldIconSize,
              imageWidth: this.animatedShieldIconSize,
            },
  ...
  getTestcoins = async () => {
    try {
      const regularAccount = await bitcoinClassState.getRegularClassState();
      const secureAccount = await bitcoinClassState.getSecureClassState();
      let resCoins = await regularAccount.getTestcoins();
  ...
  click_AccountDetails = async (item: any) => {
    try {
      ...
      this.props.navigation.push('AccountTransactionNavigator', {
        data: item,
        walletDetails,
      });
    } catch (error) {
        ...
        >
          {renderIf(item.accountType != 'Secure Account')(
            ...
            >
              ...
              >
                <ImageSVG
                  source={svgIcon.walletScreen[item.svgIcon]}
                  size={50}
                />
                <Text
                  style={[
              ...
              >
                ...
                >
                  <ImageSVG
                    ...
                    source={
                      Platform.OS == 'ios'
                        ? svgIcon.walletScreen.secureAccount2FASVG
                        : svgIcon.walletScreen.secureAccount2FAPNG
                    }
                  />
                </View>
              </View>
    ...
    return (
      <Container>
        ...
        <Content scrollEnabled={false} contentContainerStyle={styles.container}>
          ...
          >
            ...
            >
              ...
              >
                ...
                >
                  {walletDetails.walletType != null
                    ? `${walletDetails.walletType} (Testnet)`
                    : ''}
                </Animated.Text>
                <Animated.Text
                  style={[
                ...
                >
                  {arr_CustShiledIcon.length != 0
                    ? arr_CustShiledIcon[0].title
                    : ''}
                </Animated.Text>
              </Animated.View>
              ...
              >
                <ViewShieldIcons
                  data={arr_CustShiledIcon}
                  click_Image={() => {
                    const { appHealthStatus } = walletDetails;
                    if (appHealthStatus != '') {
                      const backupType = JSON.parse(
                        walletDetails.appHealthStatus,
                      );
                      if (backupType != '') {
        ...
        <ModelLoaderPdfFileCreate
          loading={flag_LoadingPdfFile}
          color={colors.appColor}
          size={30}
          msg="Creating Wallet backup. This may take a while. Just stick around."
        />
        <CustomStatusBar
          backgroundColor={colors.appColor}
          hidden={false}
