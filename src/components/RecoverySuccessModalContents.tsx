    ...
    return (<View style={{ ...styles.modalContentContainer, height: '100%' }}>
        <View style={{ height: '100%' }}>
            <View style={styles.successModalHeaderView}>
                ...
                <Text style={{ ...styles.modalInfoText, marginTop: wp('1.5%') }}>Congratulations! You can now use{"\n"}your <Text style={{ fontFamily: Fonts.FiraSansMediumItalic, fontWeight: 'bold', fontStyle: 'italic' }}>Wallet</Text></Text>
            </View>
            <View style={styles.successModalAmountView}>
                <Text style={styles.successModalWalletNameText}>{props.walletName}</Text>
                <View style={{ flexDirection: "row", alignItems: 'flex-end', }}>
                    <Image
                        style={styles.successModalAmountImage}
                        source={require('./../assets/images/icons/icon_bitcoin_gray.png')} />
                    <Text style={styles.successModalAmountText}>{props.walletAmount}</Text>
                    <Text style={styles.successModalAmountUnitText}>{props.walletUnit}</Text>
                </View>
            </View>
