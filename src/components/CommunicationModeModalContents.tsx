...
export default function CommunicationModeModalContents(props) {
    ...
    const [contactInfo, setContactInfo] = useState([
        {
            id: 1, info: "+91 000 000 0000", isSelected: false
        },
        {
            id: 2, info: "+44 0000 000000", isSelected: false
        },
        {
            id: 3, info: "sophiebabel@bithyve.com", isSelected: false
        }
    ]);
    ...
    const onContactSelect = async (index) => {
        ...
        else if (selectedContactMode.findIndex((value) => value.id == contactInfo[index].id) > -1 && !contactInfo[index].isSelected) {
            selectedContactMode.splice(selectedContactMode.findIndex((temp) => temp.id == contactInfo[index].id), 1);
        }
    ...
    return (<View style={{ ...styles.modalContentContainer, height: '100%' }}>
        <View style={{ height: '100%' }}>
            <View style={{ marginTop: hp('3.5%'), marginBottom: hp('2%'), }}>
                <Text style={styles.commModeModalHeaderText}>Select Mode of Communication{"\n"}for Contact</Text>
                <Text style={styles.commModeModalInfoText} >You can choose a primary number or email</Text>
            </View>
            ...
            <View>
                <ScrollView>
                    {contactInfo.map((item, index) => {
                        return <TouchableOpacity onPress={() => onContactSelect(index)} style={styles.contactInfo}>
                            <RadioButton size={15} color={Colors.lightBlue} borderColor={Colors.borderColor} isChecked={item.isSelected} onpress={() => onContactSelect(index)} />
