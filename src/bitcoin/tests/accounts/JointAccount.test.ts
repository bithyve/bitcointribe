...
describe.skip('Joint Account', () => {
    ...
    beforeAll(async () => {
        ...
        const cRes = await jointAccount.bitcoin.generateHDWallet(
            creatorMnemonic
        );
        creatorPriv = cRes.privateKey;

        const mRes = await jointAccount.bitcoin.generateHDWallet(
            mergerMnemonic
        );
    ...
    test("inititates the creation of joint account (cretor's end)", async () => {
        ...
        initDetails = jointAccount.initiateJointAccount(
            creatorMnemonic,
            details
        );
    ...
    test("completes an initiated joint account (creator's end)", () => {
        ...
        const {
            multiSig,
            creationJSON
        } = jointAccount.createInitiatedJointAccount(creatorMnemonic, details);
    ...
    test('finalizes an initiated transaction from the joint account (2nd signature provisioning and broadcasting)', async () => {
        const { txid } = await jointAccount.authorizeJointTxn(
            txHex,
            mergerPriv
        );
