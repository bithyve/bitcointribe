...
export const networkType = ( scannedStr: string ) => {
...
export const getKeyPair = ( privateKey: string, network: bitcoinJS.Network ): bitcoinJS.ECPairInterface =>
...
export const isValidAddress = ( address: string, network: bitcoinJS.Network ): boolean => {
  try {
...
export const getP2SH = ( keyPair: bitcoinJS.ECPairInterface, network: bitcoinJS.Network ): bitcoinJS.Payment =>
...
export const generateYpub = ( xpub: string, network: bitcoinJS.Network ): string => {
...
const isPaymentURI = ( paymentURI: string ): boolean => {
...
export const addressDiff = (
  scannedStr: string,
  network: bitcoinJS.Network
): { type: ScannedAddressKind | null } => {
  scannedStr = scannedStr.replace( 'BITCOIN', 'bitcoin' )
  if ( isPaymentURI( scannedStr ) ) {
    const { address } = decodePaymentURI( scannedStr )
    if ( isValidAddress( address, network ) ) {
      return {
        type: ScannedAddressKind.PAYMENT_URI
      }
    }
  } else if ( isValidAddress( scannedStr, network ) ) {
    return {
...
export const fetchBalanceTransactionsByAddresses = async (
...
export const getTxCounts = async ( addresses: string[], network: bitcoinJS.Network ) => {
  const txCounts = {
  }
  try {
    let res: AxiosResponse
