...
export default class HDSegwitWallet extends Bitcoin {
    ...
    constructor(
        ...
        this.nextFreeAddressIndex = stateVars
    ...
    public getReceivingAddress = async (): Promise<{ address: string }> => {
        try {
            ...
            throw new Error(
                `Unable to generate receiving address: ${err.message}`
            );
        }
    };
    ...
    public fetchBalance = async (): Promise<{
        ...
        try {
            ...
            const {
                balance,
                unconfirmedBalance
            } = await this.getBalanceByAddresses(this.usedAddresses);
        ...
        try {
            ...
            inputs.forEach(input => {
                ...
                const keyPair = this.getKeyPair(
                    this.getWifForAddress(input.address)
                );
    ...
    private getWifForAddress = (address: string): string => {
        ...
        for (const index of Object.keys(this.internalAddresssesCache)) {
            if (
                this.getInternalAddressByIndex(parseInt(index, 10)) === address
            ) {
                return (this.addressToWIFCache[
                    address
                ] = this.getInternalWIFByIndex(parseInt(index, 10)));
            }
        }

        for (const index of Object.keys(this.externalAddressesCache)) {
            if (
                this.getExternalAddressByIndex(parseInt(index, 10)) === address
            ) {
                ...
                return (this.addressToWIFCache[
                    address
                ] = this.getExternalWIFByIndex(parseInt(index, 10)));
            }
        }
