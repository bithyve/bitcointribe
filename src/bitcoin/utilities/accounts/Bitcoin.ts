...
export default class Bitcoin {
    ...
    > => {
        ...
        if (this.network === bitcoinJS.networks.testnet) {
            try {
                res = await axios.get(
                    `${TESTNET.BALANCE_CHECK}${address}/balance?token=${
                        config.TOKEN
                    }`
                );
            } catch (err) {
            ...
            try {
                res = await axios.get(
                    `${MAINNET.BALANCE_CHECK}${address}/balance?token=${
                        config.TOKEN
                    }`
                );
            } catch (err) {
        ...
        try {
            ...
            try {
                const {
                    bal,
                    unconfirmedBal
                } = await this.blockcypherBalFallback(addresses);
                return {
                    balance: bal,
    ...
    public fetchTransactionsByAddresses = async (
        ...
        try {
            ...
            for (const addressInfo of addressesInfo) {
                console.log(
                    `Appending transactions corresponding to ${
                        addressInfo.Address
                    }`
                );
            ...
            try {
                ...
                for (const address of addresses) {
                    console.log(
                        `Fetching transactions corresponding to ${address}`
                    );
                    const txns = await this.fetchTransactionsByAddress(address);
                    console.log({ txns });
                    transactions.totalTransactions +=
                        txns.transactions.totalTransactions;
                    transactions.confirmedTransactions +=
    ...
    public fundTestNetAddress = async (address: string) => {
        ...
        const funderPriv =
            'cSB5QV1Tesqtou1FDZhgfKYiEH4m55H1jT1xM8hu9xKNzWSFFgAR';
    ...
    private confirmationCat = (tx: any) => {
        ...
        const nConfirmations: number =
            tx.NumberofConfirmations || tx.confirmations;
        if (nConfirmations === 0) {
