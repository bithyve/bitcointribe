...
export default class SecureHDWallet extends Bitcoin {
    ...
    public fetchBalance = async (): Promise<{
        ...
        try {
            ...
            for (
                let itr = 0;
                itr < this.nextFreeChildIndex + this.gapLimit;
                itr++
            ) {
            ...
            const {
                balance,
                unconfirmedBalance
            } = await this.getBalanceByAddresses(this.consumedAddresses);
    ...
    public prepareSecureAccount = (
        ...
        try {
            ...
            if (!secondaryXpub) {
                secondaryXpub = this.getRecoverableXKey(
    ...
    private derivePath = (bhXpub: string): string => {
        ...
        if (bhxpub.index === 0) {
            ...
            path =
                config.SECURE_WALLET_XPUB_PATH +
                config.SECURE_DERIVATION_BRANCH;
        }
        return path;
    };
