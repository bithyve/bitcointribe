      ...
      if ( filters.channels == false ) {
        filteredActivity = filteredActivity.filter(
          ( activity: any ) =>
            !(
              activity.model ===
                            this.localeString( 'general.transaction' ) &&
      ...
      if ( filters.lightning == false ) {
        filteredActivity = filteredActivity.filter(
          ( activity: any ) =>
            !(
              activity.model ===
                            this.localeString( 'views.Invoice.title' ) ||
                        activity.model === this.localeString( 'views.Payment.title' )
            )
        )
      }
          ...
          ( activity: any ) =>
            !(
              ( activity.model ===
                            this.localeString( 'general.transaction' ) &&
                            activity.getAmount > 0 ) ||
                        activity.model === this.localeString( 'views.Invoice.title' )
            )
        )
      }
