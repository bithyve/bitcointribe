...
const TrustedContactHistory = ( props ) => {
  ...
  useEffect( () => {
    ( async () => {
      dispatch( clearTrustedContactsCache() )
      const selectedContactModeTemp = await AsyncStorage.getItem(
        'selectedContactMode',
      )
  ...
  const setContactInfo = useCallback( async () => {
    if ( trustedContactsInfo ) {
      ...
      if ( index == 1 && selectedContacts[ 0 ] ) {
        const tempContact = selectedContacts[ 0 ]
        const tcInstance =
      ...
      if ( index == 2 && selectedContacts[ 1 ] ) {
        const tempContact = selectedContacts[ 1 ]
  ...
  return (
    <View style={{
      height: '100%', backgroundColor: Colors.backgroundColor
    }}>
      <SafeAreaView
        style={{
