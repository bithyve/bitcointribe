...
> {
  ...
  setAutoHighlightFlagsFromAsync = async () => {
    ...
    if (highlightFlags) {
      this.setState({ autoHighlightFlags: JSON.parse(highlightFlags) });
    }
  };
  ...
  componentDidUpdate = (prevProps, prevState) => {
    let { s3Service, initHealthCheck } = this.props;
    ...
    if (prevProps.healthLoading != this.props.healthLoading) {
      if (this.props.healthLoading) this.setState({ showLoader: true });
      else this.setState({ showLoader: false });
    }
    if (prevProps.health != this.props.health) {
      if (this.props.health)
        this.setState({ overallHealth: this.props.health });
      else {
        (async () => {
          const storedHealth = await AsyncStorage.getItem("overallHealth");
          if (storedHealth) {
            this.setState({ overallHealth: JSON.parse(storedHealth) });
          }
        })();
      }
    }
  ...
  setContactsFromAsync = () => {
    let { trustedContactsInfo, trustedContactsService } = this.props;
  ...
  onOverAllHealth = () => {
    let { autoHighlightFlags, overallHealth } = this.state;
    ...
    if (
      ...
      this.setState({ autoHighlightFlags: updatedAutoHighlightFlags });
      // TODO -- replace this
      AsyncStorage.setItem(
        "AutoHighlightFlags",
        JSON.stringify(updatedAutoHighlightFlags)
  ...
  onChangeOverAllHealth = () => {
    let { overallHealth, pageData } = this.state;
    if (overallHealth) {
      this.setState({ isNextStepDisable: false });
  ...
  renderReshareContent = () => {
    let { SelectTypeToReshare } = this.state;
    let title = "";
    let info = "";
    let note = "";
  ...
  closePersonalCopyQRScanner = () => {
    this.setState({ LoadCamera: false, selectedType: "" });
  ...
  autoHighlight = async () => {
    let { overallHealth, autoHighlightFlags } = this.state;
    const {
      secondaryDevice,
      trustedContact1,
      trustedContact2,
