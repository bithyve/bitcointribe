...
export default function PasscodeConfirm(props) {
	...
	useEffect(() => {
		...
		if (!confirmPasscode && passcode.length == 4 && confirmPasscodeFlag==2) {
		...
		<SafeAreaView style={{ flex: 1 }}>
			...
			<View style={{ flex: 1 }}>
				<View style={{}}>
					...
					<View>
						<Text style={styles.headerInfoText}>Please enter a <Text style={styles.boldItalicText}>passcode</Text></Text>
						<TouchableWithoutFeedback disabled={passcode.length==4 && confirmPasscode.length==4} onPress={() => { setPasscodeFlag(true); setConfirmPasscodeFlag(2);}} >
							<View style={styles.passcodeTextInputView}>
								<View
									style={[passcode.length == 0 && passcodeFlag == true ? styles.textBoxActive : styles.textBoxStyles]}>
									<Text style={[passcode.length == 0 && passcodeFlag == true ? styles.textFocused : styles.textStyles]}>{passcode.length >= 1 ? <Text style={{ fontSize: RFValue(10, 812), textAlignVertical: 'center', justifyContent: 'center', alignItems: 'center' }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : passcode.length == 0 && passcodeFlag == true ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
								</View>
								<View
									style={[passcode.length == 1 ? styles.textBoxActive : styles.textBoxStyles]}>
									<Text style={[passcode.length == 1 ? styles.textFocused : styles.textStyles]}>{passcode.length >= 2 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : passcode.length == 1 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
								</View>
								<View
									style={[passcode.length == 2 ? styles.textBoxActive : styles.textBoxStyles]}>
									<Text style={[passcode.length == 2 ? styles.textFocused : styles.textStyles]}>{passcode.length >= 3 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : passcode.length == 2 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
								</View>
								<View
									style={[passcode.length == 3 ? styles.textBoxActive : styles.textBoxStyles]}>
									<Text style={[passcode.length == 3 ? styles.textFocused : styles.textStyles]}>{passcode.length >= 4 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : passcode.length == 3 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
								</View>
							</View>
						</TouchableWithoutFeedback>
					</View>
					{
						passcode.length == 4 ? <View>
							<Text style={styles.headerInfoText}>Re-enter the <Text style={styles.boldItalicText}>passcode</Text> to <Text style={styles.boldItalicText}>verify and login.</Text></Text>
							<TouchableWithoutFeedback onPress={() => { setPasscodeFlag(false); setConfirmPasscodeFlag(1); }} >
								<View style={{ flexDirection: 'row', marginTop: hp('1.5%') }}>
									<View style={[confirmPasscode.length == 0 ? styles.textBoxActive : { ...styles.textBoxStyles, borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>
										<Text style={[confirmPasscode.length == 0 && confirmPasscodeFlag == 1 ? { ...styles.textFocused, } : styles.textStyles]}>{confirmPasscode.length >= 1 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : confirmPasscode.length == 0 && confirmPasscodeFlag == 1 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
									</View>
									<View style={[confirmPasscode.length == 1 ? styles.textBoxActive : { ...styles.textBoxStyles, borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>
										<Text style={[confirmPasscode.length == 1 ? { ...styles.textFocused, borderColor: Colors.borderColor } : styles.textStyles, { borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>{confirmPasscode.length >= 2 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : confirmPasscode.length == 1 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
									</View>
									<View style={[confirmPasscode.length == 2 ? styles.textBoxActive : { ...styles.textBoxStyles, borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>
										<Text style={[confirmPasscode.length == 2 ? { ...styles.textFocused, borderColor: Colors.borderColor } : styles.textStyles, { borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>{confirmPasscode.length >= 3 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : confirmPasscode.length == 2 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
									</View>
									<View style={[confirmPasscode.length == 3 ? styles.textBoxActive : { ...styles.textBoxStyles, borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor }]}>
										<Text style={[confirmPasscode.length == 3 ? { ...styles.textFocused, borderColor: Colors.borderColor } : styles.textStyles, { borderColor: passcode != confirmPasscode && confirmPasscode.length == 4 ? Colors.red : Colors.borderColor, }]}>{confirmPasscode.length >= 4 ? <Text style={{ fontSize: RFValue(10, 812) }}><FontAwesome size={8} name={'circle'} color={Colors.black} /></Text> : confirmPasscode.length == 3 ? <Text style={styles.passcodeTextInputText}>{"|"}</Text> : ''}</Text>
									</View>
								</View>
							</TouchableWithoutFeedback>
							<TouchableOpacity
