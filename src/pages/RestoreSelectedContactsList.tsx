...
export default function RestoreSelectedContactsList(props) {
	...
	const [recoveryQuestionBottomSheet, setRecoveryQuestionBottomSheet] = useState(React.createRef());
	const [requestBottomSheet, setRequestBottomSheet] = useState(React.createRef());
	...
	function openCloseModal() {
		if (!walletName) { walletNameBottomSheet.current.snapTo(0); return }
	...
	function renderContent() {
		return <RecoveryWalletNameModalContents
			onTextChange={(text) => { setWalletName(text) }}
			onPressTextBoxFocus={() => { setWalletNameOpenModal('full') }}
			onPressTextBoxBlur={() => { setWalletNameOpenModal('half') }}
			onPressProceed={() => { walletNameBottomSheet.current.snapTo(0); recoveryQuestionBottomSheet.current.snapTo(1) }}
		/>
	}
	...
	const onPressRequest = async () => {
		...
		if (!selectedContacts[0].status && !selectedContacts[1].status) {
			...
			selectedContacts[0].status = "inTransit"; selectedContacts[1].status = "rejected";
		}
	...
	const RequestModalContentFunction = () => {
		return <RequestModalContents onPressRequest={() => onPressRequest()} onPressViaQr={() => { }} />
	}

	const RequestHeaderFunction = () => {
		return <TransparentHeaderModal onPressheader={() => { requestBottomSheet.current.snapTo(0) }} />
	}
	...
	const submitRecoveryQuestion = () => {
		recoveryQuestionBottomSheet.current.snapTo(0); successMessageBottomSheet.current.snapTo(1)
	}

	function renderRecoveryQuestionContent() {
		return <RecoveryQuestionModalContents
			onQuestionSelect={(value) => { setDropdownBoxValue(value) }}
			onTextChange={(text) => { setAnswer(text) }}
	...
	function renderSuccessContent() {
		return <RecoverySuccessModalContents
			walletName={"Pam’s Wallet"}
			walletAmount={"2,065,000"}
			walletUnit={"sats"}
			onPressGoToWallet={() => { props.navigation.navigate('Home') }}
		/>
	}
		...
		<View style={{ flex: 1 }}>
			...
			<ScrollView>
				...
				<TouchableOpacity onPress={() => props.navigation.navigate('RestoreWalletByContacts')} >
					<View style={{ ...styles.listElements, marginBottom: selectedContacts.length > 0 ? 0 : 10, }}>
					...
					{selectedContacts.length > 0 &&
						<View style={{}}>
							{selectedContacts.map((value) => {
								return <TouchableOpacity activeOpacity={value.status == "" ? 0 : 10} onPress={() => value.status == "" ? openRequestModal() : {}} style={{ ...styles.selectedContactView, marginBottom: 15 }}>
									<View>
										...
										<Text style={{ ...styles.selectedContactName, fontSize: RFValue(11, 812) }}>{value.communicationMode ?value.communicationMode[0].info :""}
								</Text>
									</View>
									{value.status == "received" ?
										(<View style={{ flexDirection: 'row', marginLeft: 'auto' }}>
											<View style={{ ...styles.secretReceivedView, backgroundColor: Colors.green, }}>
												<Text style={{ ...styles.secretReceivedText, color: Colors.darkGreen }}>Secret Receieved</Text>
											</View>
											<View style={{ ...styles.secretReceivedCheckSignView, backgroundColor: Colors.green, }}>
												<Feather name={'check'} size={12} color={Colors.darkGreen} />
											</View>
										</View>) :
										value.status == "inTransit" ?
											<View style={{ flexDirection: 'row', marginLeft: 'auto' }}>
												<View style={{ ...styles.secretReceivedView, backgroundColor: Colors.lightBlue, }}>
													<Text style={{ ...styles.secretReceivedText, color: Colors.blue, }}>Secret In-Transit</Text>
												</View>
												<View style={{ ...styles.secretReceivedCheckSignView, backgroundColor: Colors.lightBlue, }}>
													<Ionicons name={'md-time'} size={15} color={Colors.blue} />
												</View>
											</View>
											:
											value.status == "rejected" ?
												<View style={{ flexDirection: 'row', marginLeft: 'auto' }}>
													<View style={{ ...styles.secretReceivedView, backgroundColor: Colors.lightRed }}>
														<Text style={{ ...styles.secretReceivedText, color: Colors.red }}>Rejected by Contact</Text>
													</View>
													<View style={{ height: 25, width: 25, justifyContent: 'center', alignItems: 'center', marginLeft: 5 }}>
														<Entypo name={'dots-three-horizontal'} size={15} color={Colors.borderColor} />
													</View>
												</View>
												:
												<TouchableOpacity onPress={() => openRequestModal()} style={{ flexDirection: 'row', marginLeft: 'auto', }}>
				...
				<View style={styles.separator} />
				<TouchableOpacity style={{ ...styles.listElements, marginBottom: hp('5%') }} onPress={() => props.navigation.navigate('RestoreWalletUsingDocuments')}>
				...
				<View style={{ justifyContent: 'center' }}>
					<TouchableOpacity style={{ ...styles.questionConfirmButton, margin: 20 }} onPress={() => { walletNameBottomSheet.current.snapTo(1); }}>
			...
			<BottomSheet
				...
				snapPoints={[0, Platform.OS == 'ios' && DeviceInfo.hasNotch() ? hp('50%') : hp('60%'), Platform.OS == 'ios' ? hp('90%') : hp('55%')]}
			...
			<BottomSheet
				...
				snapPoints={[Platform.OS == 'ios' && DeviceInfo.hasNotch() ? 0 : 0, Platform.OS == 'ios' && DeviceInfo.hasNotch() ? hp('67%') : hp('72%'), Platform.OS == 'ios' ? hp('90%') : hp('72%')]}
			...
			<BottomSheet
				...
				snapPoints={[Platform.OS == 'ios' && DeviceInfo.hasNotch() ? 0 : 0, Platform.OS == 'ios' && DeviceInfo.hasNotch() ? hp('60%') : hp('75%')]}
