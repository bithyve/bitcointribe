...
const SecondaryDeviceHealthCheck = props => {
	...
	<Text style={BackupStyles.modalHeaderTitleText}>{props.title}</Text>
                        <Text style={BackupStyles.modalHeaderInfoText}>
                            Last backup{' '}<Text style={{ fontFamily: Fonts.FiraSansMediumItalic, fontWeight: 'bold', }}> {props.time}</Text>
                        </Text>
                </View>
                <Image style={BackupStyles.cardIconImage} source={getIconByStatus(props.status)} />
                </View>
            </View>
			...
			{SelectedOption ?
				<View style={{ justifyContent: 'center', alignItems: 'center', height: hp('25%'), backgroundColor: Colors.white }}>
					<Text style={{ marginTop: hp('1%'), marginBottom: hp('1%'), color: Colors.textColorGrey, fontSize: RFValue(10), fontFamily: Fonts.FiraSansRegular, }}>{props.reshareInfo}<Text onPress={() => { props.onPressReshare() }} style={{ color: Colors.blue, textDecorationLine: "underline" }}>Reshare</Text></Text>

					<Text style={{ marginTop: hp('1%'), marginBottom: hp('1%'), color: Colors.textColorGrey, fontSize: RFValue(10), fontFamily: Fonts.FiraSansRegular, }}>{props.changeInfo}<Text onPress={() => {props.onPressChange() }} style={{ color: Colors.blue, textDecorationLine: "underline" }}>Change Source</Text></Text>
