...
function Accounts( props ) {
  ...
  const [ RegularAccountHelperBottomSheet, setRegularAccountHelperBottomSheet ] = useState( React.createRef() );
  ...
  useEffect( () => {
    ...
    setCarouselData1();
    console.log( { selectedAccount: props.navigation.getParam( 'serviceType' ) } );
  ...
  const renderRegularAccountsHelperContents = useCallback( () => {
    return (
      <TestAccountHelperModalContents
        ...
        helperInfo={
          `\nRegular account is for your daily needs typically storing smaller amounts as compared to the Savings account.
          \n\nIt is fast and easy and does not need a second factor authentication.`
        }
        continueButtonText={ 'Continue' }
        quitButtonText={ 'Quit' }
        onPressContinue={ () => {
