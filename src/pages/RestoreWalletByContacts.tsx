...
export default function RestoreWalletByContacts(props) {
    ...
    const [communicationModeBottomSheet, setCommunicationMode] = useState(React.createRef());
    ...
    const saveCommunicationMode = async (selectedContactMode) => {
        if (contacts.length > 0) {
            if (contacts[0].communicationMode && contacts[0].communicationMode.length > 0) {
    ...
    function requestHeader() {
        return <TouchableOpacity activeOpacity={10} onPress={() => closeModal()} style={{ ...styles.modalHeaderContainer, }}>
    ...
    function renderCommunicationModeContent() {
        return <CommunicationModeModalContents onPressProceed = {(selectedContactMode)=>saveCommunicationMode(selectedContactMode)} />
    }
        ...
        <SafeAreaView style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
                ...
                <BottomSheet
                    ...
                    snapPoints={[Platform.OS == 'ios' && DeviceInfo.hasNotch() ? 0 : 0, Platform.OS == 'ios' && DeviceInfo.hasNotch() ? hp('65%') : hp('75%')]}
