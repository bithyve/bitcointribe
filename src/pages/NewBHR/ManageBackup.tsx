...
export default function ManageBackup(props) {
  ...
  useEffect(() => {
    ...
    // console.log('localleveldata', localLevelData)
  ...
  const getKeeperIcon = (item, index) => {
    let shareType = index % 2 == 0 ? item.keeper1.shareType : item.keeper2.shareType
    let status = index % 2 == 0 ? item.keeper1.status : item.keeper2.status
    let valueStatus = item.status
    let updatedAt = index % 2 == 0 ? item.keeper1.updatedAt : item.keeper2.updatedAt
    let chosenContact = index % 2 == 0 ? item.keeper1.data : item.keeper2.data
    ...
    switch (shareType) {
      ...
      case 'cloud':
        return require('../../assets/images/icons/logo_brand_brands_logos_icloud.png')

      case 'device':
      case 'primaryKeeper':
        if (status == 'accessible')
          return require('../../assets/images/icons/icon_ipad_blue.png')
        else return require('../../assets/images/icons/logo_brand_brands_logos_icloud.png')

      case 'contact':
      case 'existingContact':
        if (updatedAt != 0) {
          if (chosenContact && chosenContact.displayedName && chosenContact.avatarImageSource) {
            return { uri: chosenContact.avatarImageSource.uri ? chosenContact.avatarImageSource.uri : chosenContact.avatarImageSource }
      ...
      case 'pdf':
        ...
        else require('../../assets/images/icons/logo_brand_brands_logos_icloud.png')
                ...
                return (
                  <View style={{ flex: localLevelData.length > 2 ? 1 / 3 : 1, alignItems: 'center' }}>
                    ...
                    >
                      ...
                      <Image style={{ height: 20, width:20 }} resizeMode={'contain'} source={getKeeperIcon(item, index)} />
                    </TouchableOpacity>
                    <Text style={{ fontSize: RFValue(11), fontFamily: Fonts.FiraSansRegular, color: Colors.black, marginTop: 5, textAlign: 'center' }}>
                      {/* {index%2 == 0 ? item.keeper1.name : item.keeper2.name} */}
