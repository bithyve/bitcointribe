...
(function() {
    ...
    const globalContext =
        typeof global !== 'undefined' && global.Object === Object
            ? global
            : this;
    ...
    function defineChainableMethod(sugarNamespace, methodName, fn) {
        ...
        if (!existing || !existing.disambiguate) {
            dcp[methodName] = collision
                ? disambiguateMethod(methodName)
                : wrapped;
        }
    ...
    function buildClassChecks() {
        ...
        function addCoreTypes() {
            ...
            isSet = buildClassCheck(
                names[8],
                typeof Set !== 'undefined' && Set
            );
            isMap = buildClassCheck(
                names[9],
        ...
        function buildPrimitiveClassCheck(className) {
            ...
            return function(obj) {
                const t = typeof obj;
                return (
                    t === type || (t === 'object' && isClass(obj, className))
                );
            };
        }
    ...
    function isEqual(a, b, stack) {
        ...
        if (isSet(a, aClass) && isSet(b, bClass)) {
            return (
                a.size === b.size &&
                isEqual(setToArray(a), setToArray(b), stack)
            );
        }
        if (isMap(a, aClass) && isMap(b, bClass)) {
            return (
                a.size === b.size &&
                isEqual(mapToArray(a), mapToArray(b), stack)
            );
        }
    ...
    function objectIsEqual(a, b, aClass, stack) {
        ...
        if (isObjectType(a.valueOf())) {
            ...
            count = 0;
            propsEqual = true;
            iterateWithCyclicCheck(a, false, stack, function(
                key,
                val,
                cyc,
                stack
    ...
    function serializeDeep(obj, refs, stack) {
        let result = '';
        iterateWithCyclicCheck(obj, true, stack, function(
            key,
            val,
            cyc,
            stack
    ...
    function buildDateFormatTokens() {
        ...
        forEach(FormatTokensBase, function(f) {
            ...
            if (f.ldmlTwoDigitToken) {
                ldmlTokens[f.ldmlTwoDigitToken] = buildPadded(
                    buildTwoDigits(get),
                    2
                );
            }
            if (f.strfTwoDigitToken) {
                strfTokens[f.strfTwoDigitToken] = buildPadded(
                    buildTwoDigits(get),
                    2
                );
            }
        ...
        defineInstanceSimilar(sugarDate, 'short medium long full', function(
            methods,
            name
        ) {
    ...
    function buildDateFormatMatcher() {
        ...
        dateFormatMatcher = createFormatMatcher(
            getLdml,
            getStrf,
            checkDateToken
        );
    ...
    function getExtendedDate(contextDate, d, opt, forceClone) {
        ...
        function handleTimezoneOffset(tzHour, tzMinute, tzSign) {
            ...
            _utc(date, true);
            const offset =
                (tzSign || 1) * ((tzHour || 0) * 60 + (tzMinute || 0));
            if (offset) {
        ...
        function handleEdge(edge, params) {
            ...
            if (!edgeIndex) {
                ...
                iterateOverHigherDateParams(params, function(
                    unitName,
                    val,
                    unit,
                    i
    ...
    function updateDate(d, params, reset, advance, prefer, weekdayDir) {
        ...
        function handleFraction(unit, unitIndex, fraction) {
            if (unitIndex) {
                ...
                const val = round(
                    (unit.multiplier / lowerUnit.multiplier) * fraction
                );
        ...
        Locale.prototype = {
            ...
            addFormat(src, to) {
                ...
                function getTokenSrc(str) {
                    ...
                    if (nc) {
                        ...
                        val =
                            getOwn(loc.parsingTokens, key) || getOwn(loc, key);
                ...
                function replaceParsingTokens(str) {
                    ...
                    return str.replace(/\{([^,]+?)\}/g, function(match, token) {
                        ...
                        let src;
                        if (tokens.length > 1) {
                            src = getRegNonCapturing(
                                map(tokens, getTokenSrc).join('|')
                            );
                        } else {
            ...
            init(def) {
                ...
                function forEachAlternate(str, fn) {
                    const arr = map(str.split('+'), function(split) {
                        return split.replace(/(.+):(.+)$/, function(
                            full,
                            base,
                            suffixes
                        ) {
                ...
                function buildParsingTokens() {
                    forEachProperty(LocalizedParsingTokens, function(
                        token,
                        name
                    ) {
                        let src;
                ...
                function getTimeAfter() {
                    ...
                    markers = getRegNonCapturing(
                        markers,
                        loc.timeMarkerOptional
                    );
    ...
    function buildDateUnitMethods() {
        defineInstanceSimilar(sugarDate, DateUnits, function(
            methods,
            unit,
            index
        ) {
    ...
    function stringTitleize(str) {
        ...
        return eachWord(str, function(word, index, words) {
            ...
            isFirstOrLast =
                first || last || hasPunctuation || lastHadPunctuation;
    ...
    function collateStrings(a, b) {
        ...
        do {
            ...
            if (aValue === -1 || bValue === -1) {
                ...
                if (
                    sortNatural &&
                    codeIsNumeral(aValue) &&
                    codeIsNumeral(bValue)
                ) {
    ...
    function getSortEquivalents() {
        const equivalents = {};
        forEach(spaceSplit('AÁÀÂÃÄ CÇ EÉÈÊË IÍÌİÎÏ OÓÒÔÕÖ Sß UÚÙÛÜ'), function(
            set
        ) {
    ...
    function toQueryString(obj, deep, transform, prefix, separator) {
        if (isArray(obj)) {
            return collectArrayAsQueryString(
                obj,
                deep,
                transform,
                prefix,
    ...
    function collectArrayAsQueryString(
        arr,
        deep,
        transform,
        prefix,
    ...
    function collectObjectAsQueryString(
        obj,
        deep,
        transform,
        prefix,
        ...
        forEachProperty(obj, function(val, key) {
            ...
            result.push(
                toQueryString(val, deep, transform, fullKey, separator)
            );
        });
        return result.join('&');
    }
    ...
    function fromQueryStringWithOptions(obj, opts) {
        ...
        if (str) {
            forEach(str.split('&'), function(p) {
                ...
                const val =
                    split.length === 2 ? decodeURIComponent(split[1]) : '';
    ...
    function buildClassCheckMethods() {
        ...
        defineInstanceAndStaticSimilar(sugarObject, NATIVE_TYPES, function(
            methods,
            name,
            i
        ) {
    ...
    function median(obj, map) {
        ...
        return len % 2
            ? result[middle]
            : (result[middle - 1] + result[middle]) / 2;
    }
    ...
    function buildFromIndexMethods() {
        ...
        function createFromIndexWithOptions(methodName, opts) {
            ...
            return function(arr, startIndex, args) {
                ...
                callArgs.push(
                    applyCallback(arr, startIndex, fn, lastArg, len, loop)
                );
                if (lastArg) {
                    callArgs.push(lastArg);
                }
                result = baseFn.apply(
                    sliceArray(arr, startIndex, loop),
                    callArgs
                );
                if (getResult) {
    ...
    function abbreviateNumber(num, precision, ustr, bytes) {
        ...
        return (
            numberFormat(withPrecision(num / divisor, precision || 0)) + unit
        );
    }
    ...
    function numberFormat(num, place) {
        ...
        if (fraction) {
            result +=
                decimal +
                repeatString('0', (place || 0) - fraction.length) +
                fraction;
        }
        return (num < 0 ? '-' : '') + result;
    }
