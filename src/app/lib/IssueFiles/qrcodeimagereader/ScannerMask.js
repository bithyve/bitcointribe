...
export default class ScannerMask extends Component {
  ...
  _getStyles () {
    ...
    const _defaultScanAngleColor = props.scanAngleColor != null ? props.scanAngleColor : 'rgba(250,250,250,0.4)'
  ...
  render() {
    ...
    let hintScanText = props.hintScanText != null ? props.hintScanText : 'Put the barcode into the box'
    let hintOrText = props.hintOrText != null ? props.hintOrText : 'OR'
    let hintPictureGalleryText = props.hintPictureGalleryText != null ? props.hintPictureGalleryText : 'Choose barcode from album'
          ...
          <View style={styles.info}>
            ...
            {isEnableDiscernPicture === true ?
              <View style={{marginTop: 10, marginBottom: 10, flexDirection: 'row', alignItems: 'center'}}>
                <View style={{flex: 1, height:1, backgroundColor: 'rgba(243,248,249,0.2)', opacity: 0.6}}/>
                <Text style={{paddingLeft: 10, paddingRight: 10, color: 'rgba(243,248,249,0.2)'}}>{hintOrText}</Text>
                <View style={{flex: 1, height:1, backgroundColor: 'rgba(243,248,249,0.2)', opacity: 0.6}}/>
              </View>:''
            }
            {isEnableDiscernPicture === true ?
                ...
                >
                  <View style={{borderWidth:1,
                    ...
                    <Text style={{
                      ...
                      textAlign: 'center',
                      fontSize: 16,
                      color: '#ffffff'}}>{hintPictureGalleryText}</Text>
                  </View>
                </TouchableOpacity>
              </View>
              :''
  ...
  openGallery () {
    ...
    ImagePicker.launchImageLibrary(photoOptions,(response) => {
      if (response.didCancel) {
        ...
        readerQR(response.uri).then((data)=>{ // 读取到条码  
