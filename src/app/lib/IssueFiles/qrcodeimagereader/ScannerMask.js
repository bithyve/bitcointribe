...
export default class ScannerMask extends Component {
    ...
    _getStyles() {
        ...
        const _defaultScanAngleWidth =
            props.scanAngleWidth != null
                ? props.scanAngleWidth
                : SCAN_ANGLE_WIDTH;
        const _defaultScanAngleHeight =
            props.scanAngleHeight != null
                ? props.scanAngleHeight
                : SCAN_ANGLE_HEIGHT;
    ...
    render() {
        ...
        const isEnableTopBar =
            props.isEnableTopBar != null ? props.isEnableTopBar : true; // 是否启用背景蒙版
        const isEnableAnimationScan =
            props.isEnableAnimationScan != null
                ? props.isEnableAnimationScan
                : true; // 是否启用扫描动画

        const { RenderTopBar } = props; // 顶部自定义组件
        const Title = props.Title != null ? props.Title : 'Title'; // 顶部标题

        const isEnableDiscernPicture =
        ...
        const hintOrText = props.hintOrText != null ? props.hintOrText : 'OR';
        const hintPictureGalleryText =
            props.hintPictureGalleryText != null
                ? props.hintPictureGalleryText
                : 'Choose barcode from album';
        const { RenderScanLine } = props;
