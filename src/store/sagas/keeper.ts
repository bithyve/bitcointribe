...
function* fetchKeeperTrustedChannelWorker( { payload } ) {
  try {
    ...
    const { shareId, walletName, type } = payload
    ...
    if ( res.status == 200 ) {
      ...
      const { encryptedKey, otp } = data.shareTransferDetails
      const { bh, secondary } = data.xPub.secureXpub
      switch ( type ) {
          case 'secureXpub':
            const s3ServiceSecure: SecureAccount = yield select(
              ( state ) => state.accounts[ SECURE_ACCOUNT ].service
            )
            const secureXpubs = s3ServiceSecure.getXpubsForAccount()
            if ( secureXpubs.secondary == '' && secureXpubs.bh == '' ) {
              const response = s3ServiceSecure.setSecureXpubsAccount( secondary, bh )
            }
            let result
  ...
  try {
    ...
    if ( currentLevel == 2 && levelHealth[ 1 ] ) {
      if( keeperInfo2.findIndex( value => value.shareId == levelHealth[ 1 ].levelInfo[ 2 ].shareId ) > -1 ){
        const uuid2 = keeperInfo2[ keeperInfo2.findIndex( value => value.shareId == levelHealth[ 1 ].levelInfo[ 2 ].shareId ) ].uuid
        if( uuid2 ){
