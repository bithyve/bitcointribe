    ...
    if ( item.type == 'copy1' && type != 'Print' ) {
      if ( type == 'Other' ) {
        ...
        yield put(
          dbUpdatePdfSharing( {
            copy: 'copy1',
            socialMedia: { type: 'Other', date: Math.floor( Date.now() / 1000 ) },
          } ),
        );
      } else {
        yield Mailer.mail(
          {
            subject: item.title,
            body:
              '<b>Please find attached the personal copy 1 share pdf, it is password protected by the answer to the security question.</b>',
            isHTML: true,
            attachment: {
              path:
                Platform.OS == 'android'
        ...
        yield put(
          dbUpdatePdfSharing( {
            copy: 'copy1',
            socialMedia: { type: 'Email', date: Math.floor( Date.now() / 1000 ) },
          } ),
        );
      }
      ...
      if ( type == 'Other' ) {
        ...
        yield Mailer.mail(
          {
            subject: item.title,
            body:
              '<b>Please find attached the personal copy 2 share pdf, it is password protected by the answer to the security question.</b>',
            isHTML: true,
            attachment: {
              path:
                Platform.OS == 'android'
        ...
        yield put(
          dbUpdatePdfSharing( {
            copy: 'copy2',
            socialMedia: { type: 'Email' },
            date: Math.floor( Date.now() / 1000 ),
          } ),
        );
      }
    } else {
      console.log( { paths: databaseSSS.pdfDetails } );
      if ( item.type == 'copy1' ) {
        let pdfDecr = {
          path: databaseSSS.pdfDetails.copy1.path,
          filename: 'PersonalCopy1.pdf',
      ...
      if ( item.type == 'copy2' ) {
        ...
        if ( Platform.OS == 'android' ) {
          ...
          yield RNPrint.print( {
            filePath: databaseSSS.pdfDetails.copy2.path,
          } );
        }
      }
      ...
      yield put(
        dbUpdatePdfSharing( {
          copy: 'copy1',
          socialMedia: { type: 'Print', date: Math.floor( Date.now() / 1000 ) },
        } ),
      );
    }
...
export const dbUpdatePdfSharingWatcher = createWatcher(
  dbUPdatePdfSharingWorker,
  DBUPDATE_PDF_SEND,
);
