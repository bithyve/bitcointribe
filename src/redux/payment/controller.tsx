...
const getAccountDetails = async ({
    ...
    if (
        amount != '' &&
        parseFloat(amount) > parseFloat(accountbal) &&
        address != ''
    ) {
...
const onSendAmount = async ({
    ...
    if (arr_SelectAccountDetails.accountName == 'Regular Account') {
        resTransferST = await regularAccount.transferST1(
            address,
            amountFloat,
            priority
        );
        ...
        resTransferST = await secureAccount.transferST1(
            address,
            amountFloat,
            priority
        );
...
export { getAccountDetails, onSendAmount };
