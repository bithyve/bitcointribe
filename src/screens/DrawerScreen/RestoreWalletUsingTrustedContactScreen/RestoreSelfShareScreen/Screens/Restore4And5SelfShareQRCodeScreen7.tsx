...
export default class Restore4And5SelfShareQRCodeScreen7 extends React.Component<Props, any> {
    ...
    barcodeReceived( e: any ) {
        try {
            ...
            if ( type == "iCloud" ) {
                if ( firstChar == "c07" ) {
                    if ( flag_ReadQRCode ) {
                        this.props.click_Next( 7, [ result ] );
                        flag_ReadQRCode = false;
                    }
                } else {
                    if ( flag_ReadQRCode ) {
                        alert.simpleOkAction( "Oops", "Please scan share 7 qrcode.", this.click_resetFlag );
                        flag_ReadQRCode = false;
                    }
                }
            } else {
