...
export default class Restore4And5SelfShareQRCodeScreen5 extends React.Component<Props, any> {
        ...
        try {
            ...
            if ( type == "iCloud" ) {
                if ( firstChar == "c05" ) {
                    if ( flag_ReadQRCode ) {
                        console.log( { result } );
                        this.props.click_Next( 5, [ result ] );
                        flag_ReadQRCode = false;
                    }
                } else {
                    if ( flag_ReadQRCode ) {
                        alert.simpleOkAction( "Oops", "Please scan share 5 qrcode.", this.click_resetFlag );
                        flag_ReadQRCode = false;
                    }
                }
            } else {
            ...
            <Container>
                <View style={ styles.container }>
                    { renderIf( flag_qrcode == false )(
                        ...
                        >
                            <Text style={ { color: "#000000" } }>Tab To Scan share 5</Text>
                        </Button>
                    ) }
