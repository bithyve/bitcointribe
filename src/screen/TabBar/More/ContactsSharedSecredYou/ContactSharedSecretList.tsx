...
export default class ContactSharedSecretList extends React.Component<any, any> {
  ...
  async componentWillMount(flagModelOpen: boolean) {
    ...
    if (urlType == 'SSS Restore SMS/EMAIL') {
      ...
      alert.simpleOkAction(
        'Request',
        message,
        this.click_RemoveDeeplinkingData,
      );
    }
  }
  ...
  load_data = async (data: any) => {
    ...
    const resEncryptViaOTP = S3Service.encryptViaOTP(
      resGenerateEncryptedMetaShare.key,
    );
  ...
  click_SentRequest(type: string, val: any) {
    ...
    if (type == 'SMS') {
      ...
      this.props.navigation.push('TrsutedPartyQRCode', {
            ...
            >
              ...
              >
                ...
                >
                  {renderIf(item.keeperInfo.thumbnailPath != '')(
                    <Avatar
                      medium
                      rounded
                      source={{ uri: item.keeperInfo.thumbnailPath }}
                    />,
                  )}
                  {renderIf(item.keeperInfo.thumbnailPath == '')(
                    <Avatar
                      ...
                      rounded
                      title={
                        item.keeperInfo.givenName != null &&
                        item.keeperInfo.givenName.charAt(0)
                      }
                    />,
                  )}
                </View>
                <View style={{ flex: 1, flexDirection: 'column' }}>
                  <Text
                    style={[FontFamily.ffFiraSansMedium, { marginLeft: 10 }]}
                  >
                    {item.name}
                  </Text>
                  <Text
                    style={[FontFamily.ffFiraSansRegular, { marginLeft: 10 }]}
                  >
                    {item.mobileNo != '' ? item.mobileNo : 'Not Number!'}
                  </Text>
                  <Text
                    note
                    style={[FontFamily.ffFiraSansRegular, { marginLeft: 10 }]}
                  ...
                  {renderIf(typeof item.opt !== 'undefined')(
                    <TimerCountdown
                      ...
                      style={[
                        ...
                        { marginLeft: 10, fontSize: 14, color: 'gray' },
                      ]}
                    />,
                  )}
                  {renderIf(typeof item.opt !== 'undefined')(
                ...
                >
                  {renderIf(item.type != 'Self Share')(
                    <IconFontAwe
                      name="angle-down"
                      style={{ fontSize: 25, marginRight: 10, flex: 0.1 }}
                    />,
                  )}
                  {renderIf(item.type == 'Self Share')(
                    <IconFontAwe
              ...
              <View style={{ flex: 0.2 }}>
                ...
                >
                  <Item style={{ borderColor: 'transparent', marginLeft: 10 }}>
                    ...
                    <Input
                      placeholder="Enter a user name or wallet name"
                ...
                >
                  Click on wallet name of trusted contact to send back share
                </Text>
              </View>
              <View style={{ flex: 1 }}>{secretList}</View>
                ...
                >
                  {renderIf(arr_SelectedContact.thumbnailPath != '')(
                    <Avatar
                      medium
                      rounded
                      source={{ uri: arr_SelectedContact.thumbnailPath }}
                    />,
                  )}
                  {renderIf(arr_SelectedContact.thumbnailPath == '')(
                    <Avatar
                      ...
                      rounded
                      title={
                        arr_SelectedContact.givenName != null &&
                        arr_SelectedContact.givenName.charAt(0)
                      }
                    />,
                  )}
                ...
                <View style={{ alignItems: 'center' }}>
                  <View style={{ flexDirection: 'row', marginBottom: 10 }}>
                    <Button
                      ...
                      onPress={() =>
                        this.click_SentRequest(
                          'SMS',
                          arr_SelectedContact.phoneNumbers,
                        )
                      }
                    >
                      <View
                    ...
                    <Button
                      ...
                      onPress={() =>
                        this.click_SentRequest(
                          'EMAIL',
                          arr_SelectedContact.emailAddresses,
                        )
                      }
                    >
                      <View
                    ...
                    <Button
                      ...
                      onPress={() =>
                        this.click_SentRequest(
                          'QR',
                          arr_SelectedContact.qrCodeString,
                        )
                      }
                    >
                      <View
